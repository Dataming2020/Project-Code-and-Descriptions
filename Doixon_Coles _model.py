...
def solve_parameters(dataset, debug = False, init_vals=None, options={'disp': True, 'maxiter':100},
                     constraints = [{'type':'eq', 'fun': lambda x: sum(np.exp(x[:20]))-20}] , **kwargs):
...
#                    params, row.HomeTeam, row.AwayTeam))[row.FTR]) for row in test_dataset.itertuples()])
    prediction = [get_1x2_probs(dixon_coles_simulate_match(
                    params, row.HomeTeam, row.AwayTeam)) for row in test_dataset.itertuples()]
    return prediction  

def get_total_score_xi(xi):
    xi_result = [build_temp_model(epl_1920, day, xi=xi) for day in range(99,-1,-3)]
    with open('paramater_1season_{}.txt'.format(str(xi)[2:]), 'wb') as thefile:
        pickle.dump(xi_result, thefile)
    return xi_result

epl_1920 = pd.read_csv("http://www.football-data.co.uk/mmz4281/1920/E0.csv")
epl_1920['Date'] = pd.to_datetime(epl_1920['Date'],  dayfirst = True)
epl_1920['time_diff'] = (max(epl_1920['Date']) - epl_1920['Date']).dt.days
epl_1920 = epl_1920[['HomeTeam','AwayTeam','FTHG','FTAG', 'FTR', 'time_diff']]
...
#        xi_result = get_total_score_xi(0.00325)
         xi_result = build_temp_model(epl_1920, 0, xi=0.00325)
         with open('Prediction_1season_{}.txt'.format(str(0.00325)[2:]), 'wb') as thefile:
             pickle.dump(xi_result, thefile)
